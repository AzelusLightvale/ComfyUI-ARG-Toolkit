...
This node takes two inputs, interprets them as byte arrays based on the selected `datatype`, and performs a bitwise AND operation between them. The inputs must be of the same length. The result is then formatted back into the same datatype as the input.
...
- **bitwise_txt**: The result of the AND operation, formatted according to the input `datatype`.
