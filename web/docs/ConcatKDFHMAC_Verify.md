...
Verifies a key derived with the ConcatKDF (HMAC) algorithm.

Source library: `cryptography`

## Overview

This node re-derives a key using the same input parameters as the `ConcatKDF (HMAC) Key Derivation` node and compares it in constant time to an expected key.
